...
#define EP11SHAREDLIB "libgm11.so"
...
static gm_GenerateRandom_t  dll_gm_GenerateRandom;
static gm_SeedRandom_t      dll_gm_SeedRandom;

static gm_Digest_t          dll_gm_Digest;
static gm_DigestInit_t      dll_gm_DigestInit;
static gm_DigestUpdate_t    dll_gm_DigestUpdate;
static gm_DigestKey_t       dll_gm_DigestKey;
static gm_DigestFinal_t     dll_gm_DigestFinal;
static gm_DigestSingle_t    dll_gm_DigestSingle;

static gm_Encrypt_t         dll_gm_Encrypt;
static gm_EncryptInit_t     dll_gm_EncryptInit;
static gm_EncryptUpdate_t   dll_gm_EncryptUpdate;
static gm_EncryptFinal_t    dll_gm_EncryptFinal;
static gm_EncryptSingle_t   dll_gm_EncryptSingle;

static gm_Decrypt_t         dll_gm_Decrypt;
static gm_DecryptInit_t     dll_gm_DecryptInit;
static gm_DecryptUpdate_t   dll_gm_DecryptUpdate;
static gm_DecryptFinal_t    dll_gm_DecryptFinal;
static gm_DecryptSingle_t   dll_gm_DecryptSingle;

static gm_ReencryptSingle_t dll_gm_ReencryptSingle;
static gm_GenerateKey_t     dll_gm_GenerateKey;
static gm_GenerateKeyPair_t dll_gm_GenerateKeyPair;

static gm_Sign_t            dll_gm_Sign;
static gm_SignInit_t        dll_gm_SignInit;
static gm_SignUpdate_t      dll_gm_SignUpdate;
static gm_SignFinal_t       dll_gm_SignFinal;
static gm_SignSingle_t      dll_gm_SignSingle;

static gm_Verify_t          dll_gm_Verify;
static gm_VerifyInit_t      dll_gm_VerifyInit;
static gm_VerifyUpdate_t    dll_gm_VerifyUpdate;
static gm_VerifyFinal_t     dll_gm_VerifyFinal;
static gm_VerifySingle_t    dll_gm_VerifySingle;

static gm_WrapKey_t         dll_gm_WrapKey;
static gm_UnwrapKey_t       dll_gm_UnwrapKey;
static gm_DeriveKey_t       dll_gm_DeriveKey;

static gm_GetMechanismList_t    dll_gm_GetMechanismList;
static gm_GetMechanismInfo_t    dll_gm_GetMechanismInfo;
static gm_GetAttributeValue_t   dll_gm_GetAttributeValue;
static gm_SetAttributeValue_t   dll_gm_SetAttributeValue;

static gm_Login_t       dll_gm_Login;
static gm_Logout_t      dll_gm_Logout;
static gm_admin_t       dll_gm_admin;
static gm_add_backend_t dll_gm_add_backend;
static gm_init_t        dll_gm_init;
static gm_shutdown_t    dll_gm_shutdown;
...
CK_CHAR manuf[] = "Softway";
CK_CHAR model[] = "Softway GM11Tok";
CK_CHAR descr[] = "Softway PKCS#11 GM11 token";
CK_CHAR label[] = "Softway OS PKCS#11";
...
char     wrap_key_name[] = "GM11_wrapkey";

/* EP11 token private data */
typedef struct {
    uint64_t      *target_list; // pointer to adapter target list
    unsigned char *gm11_pin_blob;
    CK_ULONG      gm11_pin_blob_len;
...
} __attribute__((packed)) gm11_target_t;

/* defined in the makefile, gm11 library can run standalone (without HW card),
 ...
 */
CK_RV
gm11_spki_key(CK_BYTE *spki, CK_BYTE **key, CK_ULONG *bit_str_len) {
...
CK_RV
gm11_get_keytype(CK_ATTRIBUTE *attrs, CK_ULONG attrs_len,
...
static const char* gm11_get_ckm(CK_ULONG mechanism) {
    switch(mechanism) {
    case CKM_RSA_PKCS_KEY_PAIR_GEN:
    ...
    case CKA_IBM_MACKEY:
        return "CKA_IBM_MACKEY";
    case CKM_IBM_ECDSA_SHA224:
        return "CKM_IBM_ECDSA_SHA224";
    case CKM_IBM_ECDSA_SHA256:
        return "CKM_IBM_ECDSA_SHA256";
    case CKM_IBM_ECDSA_SHA384:
        return "CKM_IBM_ECDSA_SHA384";
    case CKM_IBM_ECDSA_SHA512:
        return "CKM_IBM_ECDSA_SHA512";
    case CKM_IBM_EC_MULTIPLY:
        return "CKM_IBM_EC_MULTIPLY";
    case CKM_IBM_EAC:
        return "CKM_IBM_EAC";
    case CKM_IBM_SHA512_256:
        return "CKM_IBM_SHA512_256";
    case CKM_IBM_SHA512_224:
        return "CKM_IBM_SHA512_224";
    case CKM_IBM_SHA512_256_HMAC:
        return "CKM_IBM_SHA512_256_HMAC";
    case CKM_IBM_SHA512_224_HMAC:
        return "CKM_IBM_SHA512_224_HMAC";
    case CKM_IBM_SHA512_256_KEY_DERIVATION:
        return "CKM_IBM_SHA512_256_KEY_DERIVATION";
    case CKM_IBM_SHA512_224_KEY_DERIVATION:
        return "CKM_IBM_SHA512_224_KEY_DERIVATION";
    case CKM_IBM_ATTRIBUTEBOUND_WRAP:
        return "CKM_IBM_ATTRIBUTEBOUND_WRAP";
    case CKM_IBM_TRANSPORTKEY:
        return "CKM_IBM_TRANSPORTKEY";
    case CKM_IBM_DH_PKCS_DERIVE_RAW:
        return "CKM_IBM_DH_PKCS_DERIVE_RAW";
    case CKM_IBM_ECDH1_DERIVE_RAW:
...
#define EP11_DEFAULT_CFG_FILE "gm11tok.conf"
...
static CK_RV rawkey_2_blob(STDLL_TokData_t  * tokdata, unsigned char *key,
    ...
    gm11_private_data_t *gm11_data = tokdata->private_data;
    ...
    /* tell gm11 the attributes the user specified */
    node = key_obj->template->attribute_list;
    while (node != NULL) {
        CK_ATTRIBUTE_PTR a = node->data;

        /* gm11 handles this as 'read only' and reports
    ...
    /*
     * calls the gm11 lib (which in turns sends the request to the card),
     * all gm_ function are gm11 functions
     */
    rc = dll_gm_EncryptSingle(gm11_data->raw2key_wrap_blob, gm11_data->raw2key_wrap_blob_l, &mech,
                              key, ksize, cipher, &clen, (uint64_t)gm11_data->target_list);
     ...
     */
    rc = dll_gm_UnwrapKey(cipher, clen, gm11_data->raw2key_wrap_blob, gm11_data->raw2key_wrap_blob_l,
                          NULL, ~0, gm11_data->gm11_pin_blob, gm11_data->gm11_pin_blob_len, &mech,
                          new_p_attrs, new_attrs_len, blob, blen, csum, &cslen,
                          (uint64_t)gm11_data->target_list);
...
CK_RV token_specific_rng(STDLL_TokData_t *tokdata, CK_BYTE *output, CK_ULONG bytes) {
    ...
    CK_RV rc = dll_gm_GenerateRandom(output, bytes,
                                     (uint64_t)gm11_data->target_list);
    if (rc != CKR_OK)
 ...
 * gm_UnwrapKey, use one wrap key for this purpose, can be any key,
 * we use an AES key
 */
static CK_RV make_wrapblob(STDLL_TokData_t *tokdata, CK_ATTRIBUTE *tmpl_in,
    ...
    if (gm11_data->raw2key_wrap_blob_l != 0) {
        TRACE_INFO("%s blob already exists raw2key_wrap_blob_l=0x%zx\n",
                   __func__, gm11_data->raw2key_wrap_blob_l);
        return CKR_OK;
    }

    gm11_data->raw2key_wrap_blob_l = sizeof(gm11_data->raw2key_wrap_blob);
    rc = dll_gm_GenerateKey(&mech, tmpl_in, tmpl_len, NULL, 0, gm11_data->raw2key_wrap_blob,
                            &gm11_data->raw2key_wrap_blob_l, csum, &csum_l,
                            (uint64_t)gm11_data->target_list);


    if (rc != CKR_OK) {
        TRACE_ERROR("%s end raw2key_wrap_blob_l=0x%zx rc=0x%lx\n",
                    __func__, gm11_data->raw2key_wrap_blob_l, rc);
    } else {
...
CK_RV gm11_resolve_lib_sym(void *hdl) {
    char *error = NULL;

    dlerror(); /* Clear existing error */

    dll_gm_GenerateRandom   = (gm_GenerateRandom_t)dlsym(hdl, "gm_GenerateRandom");
    dll_gm_SeedRandom       = (gm_SeedRandom_t)dlsym(hdl, "gm_SeedRandom");

    dll_gm_Digest           = (gm_Digest_t)dlsym(hdl, "gm_Digest");
    dll_gm_DigestInit       = (gm_DigestInit_t)dlsym(hdl, "gm_DigestInit");
    dll_gm_DigestUpdate     = (gm_DigestUpdate_t)dlsym(hdl, "gm_DigestUpdate");
    dll_gm_DigestFinal      = (gm_DigestFinal_t)dlsym(hdl, "gm_DigestFinal");
    dll_gm_DigestKey        = (gm_DigestKey_t)dlsym(hdl, "gm_DigestKey");
    dll_gm_DigestSingle     = (gm_DigestSingle_t)dlsym(hdl, "gm_DigestSingle");

    dll_gm_Encrypt          = (gm_Encrypt_t)dlsym(hdl, "gm_Encrypt");
    dll_gm_EncryptInit      = (gm_EncryptInit_t)dlsym(hdl, "gm_EncryptInit");
    dll_gm_EncryptUpdate    = (gm_EncryptUpdate_t)dlsym(hdl, "gm_EncryptUpdate");
    dll_gm_EncryptFinal     = (gm_EncryptFinal_t)dlsym(hdl, "gm_EncryptFinal");
    dll_gm_EncryptSingle    = (gm_EncryptSingle_t)dlsym(hdl, "gm_EncryptSingle");

    dll_gm_Decrypt          = (gm_Decrypt_t)dlsym(hdl, "gm_Decrypt");
    dll_gm_DecryptInit      = (gm_DecryptInit_t)dlsym(hdl, "gm_DecryptInit");
    dll_gm_DecryptUpdate    = (gm_DecryptUpdate_t)dlsym(hdl, "gm_DecryptUpdate");
    dll_gm_DecryptFinal     = (gm_DecryptFinal_t)dlsym(hdl, "gm_DecryptFinal");
    dll_gm_DecryptSingle    = (gm_DecryptSingle_t)dlsym(hdl, "gm_DecryptSingle");

    dll_gm_ReencryptSingle  = (gm_ReencryptSingle_t)dlsym(hdl, "gm_ReencryptSingle");
    dll_gm_GenerateKey      = (gm_GenerateKey_t)dlsym(hdl, "gm_GenerateKey");
    dll_gm_GenerateKeyPair  = (gm_GenerateKeyPair_t)dlsym(hdl, "gm_GenerateKeyPair");

    dll_gm_Sign         = (gm_Sign_t)dlsym(hdl, "gm_Sign");
    dll_gm_SignInit     = (gm_SignInit_t)dlsym(hdl, "gm_SignInit");
    dll_gm_SignUpdate   = (gm_SignUpdate_t)dlsym(hdl, "gm_SignUpdate");
    dll_gm_SignFinal    = (gm_SignFinal_t)dlsym(hdl, "gm_SignFinal");
    dll_gm_SignSingle   = (gm_SignSingle_t)dlsym(hdl, "gm_SignSingle");

    dll_gm_Verify       = (gm_Verify_t)dlsym(hdl, "gm_Verify");
    dll_gm_VerifyInit   = (gm_VerifyInit_t)dlsym(hdl, "gm_VerifyInit");
    dll_gm_VerifyUpdate = (gm_VerifyUpdate_t)dlsym(hdl, "gm_VerifyUpdate");
    dll_gm_VerifyFinal  = (gm_VerifyFinal_t)dlsym(hdl, "gm_VerifyFinal");
    dll_gm_VerifySingle = (gm_VerifySingle_t)dlsym(hdl, "gm_VerifySingle");

    dll_gm_WrapKey      = (gm_WrapKey_t)dlsym(hdl, "gm_WrapKey");
    dll_gm_UnwrapKey    = (gm_UnwrapKey_t)dlsym(hdl, "gm_UnwrapKey");
    dll_gm_DeriveKey    = (gm_DeriveKey_t)dlsym(hdl, "gm_DeriveKey");

    dll_gm_GetMechanismList     = (gm_GetMechanismList_t)dlsym(hdl, "gm_GetMechanismList");
    dll_gm_GetMechanismInfo     = (gm_GetMechanismInfo_t)dlsym(hdl, "gm_GetMechanismInfo");
    dll_gm_GetAttributeValue    = (gm_GetAttributeValue_t)dlsym(hdl, "gm_GetAttributeValue");
    dll_gm_SetAttributeValue    = (gm_SetAttributeValue_t)dlsym(hdl, "gm_SetAttributeValue");

    dll_gm_Login        = (gm_Login_t)dlsym(hdl, "gm_Login");
    dll_gm_Logout       = (gm_Logout_t)dlsym(hdl, "gm_Logout");
    dll_gm_admin        = (gm_admin_t)dlsym(hdl, "gm_admin");

    dll_gm_init         = (gm_init_t)dlsym(hdl, "gm_init");
    dll_gm_add_backend  = (gm_add_backend_t)dlsym(hdl, "gm_add_backend");
    dll_gm_shutdown     = (gm_shutdown_t)dlsym(hdl, "gm_shutdown");
...
CK_RV gm11tok_init(STDLL_TokData_t *tokdata, CK_SLOT_ID SlotNumber, char *conf_name) {
    CK_RV rc;
    void *lib_gm11;
    CK_ULONG len = 16;
    CK_BBOOL cktrue = 1;
    ...
    gm11_private_data_t *gm11_data;

    TRACE_INFO("gm11 %s slot=%lu running\n", __func__, SlotNumber);

    gm11_data = calloc(1, sizeof(gm11_private_data_t));
    if (gm11_data == NULL)
        return CKR_HOST_MEMORY;
    gm11_data->target_list = calloc(1, sizeof(gm11_target_t));
    if (gm11_data->target_list == NULL)
        return CKR_HOST_MEMORY;

    tokdata->private_data = gm11_data;

    /* read gm11 specific config file with user specified adapter/domain pairs, ... */
    rc = read_adapter_config_file(tokdata, conf_name);
    if (rc != CKR_OK) {
        TRACE_ERROR("%s gm11 config file error rc=0x%lx\n", __func__, rc);
        return CKR_GENERAL_ERROR;
    }

    /* dynamically load in the gm11 shared library */
    lib_gm11 = dlopen(EP11SHAREDLIB, RTLD_GLOBAL | RTLD_NOW);
    if (!lib_gm11) {
        OCK_SYSLOG(LOG_ERR,
    ...
    rc = gm11_resolve_lib_sym(lib_gm11);
    if (rc)
        exit(rc);

#ifndef XCP_STANDALONE
    /* call gm11 shared lib init */
    if (dll_gm_init() < 0) {
        TRACE_ERROR("%s gm11 lib init failed\n", __func__);
        return CKR_DEVICE_ERROR;
    }
#endif

    /* create an AES key needed for importing keys
     * (encrypt by wrap_key and gm_UnwrapKey by wrap key)
     */
...
CK_RV gm11tok_final(STDLL_TokData_t *tokdata) {
    ...
    TRACE_INFO("gm11 %s running\n", __func__);

    if (gm11_data != NULL) {
        if (gm11_data->target_list)
            free(gm11_data->target_list);
        free(gm11_data);
...
static CK_RV import_RSA_key(STDLL_TokData_t *tokdata, OBJECT *rsa_key_obj,
    ...
    /* gm_Unwrap builds key blob in the card,
     * tell gm11 the attributes the user specified for that key.
     */
    node = rsa_key_obj->template->attribute_list;
    while (node != NULL) {
        CK_ATTRIBUTE_PTR a = node->data;

        /* gm11 handles this as 'read only' */
    ...
    if (class != CKO_PRIVATE_KEY) {
        ...
        *blob_size = data_len;
        ...
        /* encrypt */
        rc = dll_gm_EncryptSingle(gm11_data->raw2key_wrap_blob, gm11_data->raw2key_wrap_blob_l, &mech_w,
         ...
         */
        rc = dll_gm_UnwrapKey(cipher, cipher_l, gm11_data->raw2key_wrap_blob, gm11_data->raw2key_wrap_blob_l,
                              NULL, ~0, gm11_data->gm11_pin_blob, gm11_data->gm11_pin_blob_len, &mech_w,
...
CK_RV gm11tok_generate_key(STDLL_TokData_t *tokdata, SESSION *session,
    ...
    rc = gm11_get_keytype(attrs, attrs_len, mech, &ktype, &class);
    if (rc != CKR_OK) {
    ...
    rc = dll_gm_GenerateKey(mech, new_attrs, new_attrs_len, gm11_data->gm11_pin_blob,
                            gm11_data->gm11_pin_blob_len, blob, &blobsize,
                            csum, &csum_len, (uint64_t)gm11_data->target_list);
    if (rc != CKR_OK) {
        TRACE_ERROR("%s gm_GenerateKey rc=0x%lx mech='%s' attrs_len=0x%lx\n",
                    __func__, rc, gm11_get_ckm(mech->mechanism), attrs_len);
        return rc;
    }

    TRACE_INFO("%s gm_GenerateKey rc=0x%lx mech='%s' attrs_len=0x%lx\n",
               __func__, rc, gm11_get_ckm(mech->mechanism), attrs_len);
...
CK_RV token_specific_sha_init(STDLL_TokData_t *tokdata, DIGEST_CONTEXT *c,
    ...
    rc = dll_gm_DigestInit (state, &state_len, mech,
                            (uint64_t)gm11_data->target_list);
...
CK_RV token_specific_sha(STDLL_TokData_t *tokdata, DIGEST_CONTEXT *c,
    ...
    rc = dll_gm_Digest(c->context, c->context_len, in_data, in_data_len,
                       out_data, out_data_len, (uint64_t)gm11_data->target_list);
...
CK_RV token_specific_sha_update(STDLL_TokData_t *tokdata, DIGEST_CONTEXT *c,
    ...
    rc = dll_gm_DigestUpdate(c->context, c->context_len, in_data, in_data_len,
                             (uint64_t)gm11_data->target_list) ;
...
CK_RV token_specific_sha_final(STDLL_TokData_t *tokdata, DIGEST_CONTEXT *c,
    ...
    rc = dll_gm_DigestFinal(c->context, c->context_len, out_data, out_data_len,
                            (uint64_t)gm11_data->target_list) ;
...
CK_RV gm11tok_derive_key(STDLL_TokData_t *tokdata, SESSION *session, CK_MECHANISM_PTR mech,
    ...
    rc = dll_gm_DeriveKey (mech, new_attrs, new_attrs_len, keyblob, keyblobsize, NULL,
                           0, gm11_data->gm11_pin_blob, gm11_data->gm11_pin_blob_len, newblob, &newblobsize,
                           csum, &cslen, (uint64_t)gm11_data->target_list);
...
static CK_RV dh_generate_keypair(STDLL_TokData_t *tokdata,
    ...
    /* gm11 accepts CKA_PRIME and CKA_BASE parameters/attributes
     * only in this format
     */
    struct {
    ...
    rc = dll_gm_GenerateKeyPair(pMechanism, pPublicKeyTemplate_new,
                                new_public_attr+1, pPrivateKeyTemplate,
                                ulPrivateKeyAttributeCount, gm11_data->gm11_pin_blob,
                                gm11_data->gm11_pin_blob_len, privblob, &privblobsize,
                                publblob, &publblobsize,
                                (uint64_t)gm11_data->target_list);

    if (rc != CKR_OK) {
        TRACE_ERROR("%s gm_GenerateKeyPair failed rc=0x%lx\n", __func__, rc);
        goto dh_generate_keypair_end;
    }
    ...
    rc = gm11_spki_key(publblob, &y_start, &bit_str_len);
    if (rc != CKR_OK) {
...
static CK_RV dsa_generate_keypair(STDLL_TokData_t *tokdata,
    ...
    /* gm11 accepts CKA_PRIME,CKA_SUBPRIME,CKA_BASE only in this format */
    struct {
    ...
    rc = dll_gm_GenerateKeyPair(pMechanism, dsa_pPublicKeyTemplate,
                                dsa_ulPublicKeyAttributeCount,
                                dsa_pPrivateKeyTemplate,
                                dsa_ulPrivateKeyAttributeCount, gm11_data->gm11_pin_blob,
                                gm11_data->gm11_pin_blob_len, privblob, &privblobsize,
                                publblob, &publblobsize,
                                (uint64_t)gm11_data->target_list);

    if (rc != CKR_OK) {
        TRACE_ERROR("%s gm_GenerateKeyPair failed with rc=0x%lx\n", __func__, rc);
        goto dsa_generate_keypair_end;
    }
    ...
    rc = gm11_spki_key(publblob, &key, &bit_str_len);
    if (rc != CKR_OK) {
...
static CK_RV rsa_ec_generate_keypair(STDLL_TokData_t *tokdata,
    ...
    rc = dll_gm_GenerateKeyPair(pMechanism, new_pPublicKeyTemplate,
                                new_ulPublicKeyAttributeCount, new_pPrivateKeyTemplate,
                                new_ulPrivateKeyAttributeCount, gm11_data->gm11_pin_blob,
                                gm11_data->gm11_pin_blob_len, privkey_blob,
                                &privkey_blob_len, spki, &spki_len,
                                (uint64_t)gm11_data->target_list);
    if (rc != CKR_OK) {
        TRACE_ERROR("%s gm_GenerateKeyPair rc=0x%lx spki_len=0x%zx "
                    "privkey_blob_len=0x%zx mech='%s'\n",
                    __func__, rc, spki_len, privkey_blob_len,
                    gm11_get_ckm(pMechanism->mechanism));
        goto error;
    }
    TRACE_INFO("%s gm_GenerateKeyPair rc=0x%lx spki_len=0x%zx "
               "privkey_blob_len=0x%zx mech='%s'\n",
               __func__, rc, spki_len, privkey_blob_len,
               gm11_get_ckm(pMechanism->mechanism));
...
#endif
        rc = gm11_spki_key(spki, &key, &bit_str_len);
        if (rc != CKR_OK) {
...
CK_RV gm11tok_generate_key_pair(STDLL_TokData_t *tokdata, SESSION * sess,
    ...
    /* Get the keytype to use when creating the key object */
    rc = gm11_get_keytype(pPrivateKeyTemplate, ulPrivateKeyAttributeCount,
    ...
    rc = gm11_get_keytype(pPublicKeyTemplate, ulPublicKeyAttributeCount,
...
CK_RV gm11tok_sign_init(STDLL_TokData_t *tokdata, SESSION *session,
    ...
    size_t gm11_sign_state_l = MAX_SIGN_STATE_BYTES;
    CK_BYTE *gm11_sign_state = malloc(gm11_sign_state_l);
    ...
    rc = dll_gm_SignInit(gm11_sign_state, &gm11_sign_state_l,
    ...
    if (rc != CKR_OK) {
        ...
        ctx->multi = FALSE;
        ctx->active = TRUE;
        ctx->context = gm11_sign_state;
        ctx->context_len = gm11_sign_state_l;
...
CK_RV gm11tok_sign(STDLL_TokData_t *tokdata, SESSION *session,
    ...
    rc = dll_gm_Sign(ctx->context, ctx->context_len, in_data, in_data_len,
                     signature, sig_len, (uint64_t)gm11_data->target_list);
...
CK_RV gm11tok_sign_update(STDLL_TokData_t *tokdata, SESSION *session,
    ...
    rc = dll_gm_SignUpdate(ctx->context, ctx->context_len, in_data,
                           in_data_len, (uint64_t)gm11_data->target_list);
...
CK_RV gm11tok_sign_final(STDLL_TokData_t *tokdata, SESSION *session,
    ...
    rc = dll_gm_SignFinal(ctx->context, ctx->context_len, signature, sig_len,
                          (uint64_t)gm11_data->target_list);
...
CK_RV gm11tok_verify_init(STDLL_TokData_t *tokdata, SESSION *session,
    ...
    rc = dll_gm_VerifyInit(gm11_sign_state, &gm11_sign_state_l, mech,
                           spki, spki_len, (uint64_t)gm11_data->target_list);

    if (rc != CKR_OK) {
        TRACE_ERROR("%s rc=0x%lx spki_len=0x%zx key=0x%lx "
                    "gm11_sign_state_l=0x%zx mech=0x%lx\n", __func__,
                    rc, spki_len, key, gm11_sign_state_l,
                    mech->mechanism);
    } else {
        ctx->key = key;
        ctx->multi = FALSE;
...
CK_RV gm11tok_verify(STDLL_TokData_t *tokdata, SESSION *session, CK_BYTE *in_data,
                     CK_ULONG in_data_len, CK_BYTE *signature, CK_ULONG sig_len) {
    gm11_private_data_t *gm11_data = tokdata->private_data;
    CK_RV rc;
    SIGN_VERIFY_CONTEXT *ctx = &session->verify_ctx;

    rc = dll_gm_Verify(ctx->context, ctx->context_len, in_data, in_data_len,
                       signature, sig_len, (uint64_t)gm11_data->target_list);
...
CK_RV gm11tok_verify_update(STDLL_TokData_t *tokdata, SESSION *session,
    ...
    rc = dll_gm_VerifyUpdate(ctx->context, ctx->context_len, in_data,
                             in_data_len, (uint64_t)gm11_data->target_list);
...
CK_RV gm11tok_verify_final(STDLL_TokData_t *tokdata, SESSION *session,
    ...
    rc = dll_gm_VerifyFinal(ctx->context, ctx->context_len, signature,
                            sig_len, (uint64_t)gm11_data->target_list);
...
CK_RV gm11tok_decrypt_final(STDLL_TokData_t *tokdata, SESSION *session,
    ...
    rc = dll_gm_DecryptFinal(ctx->context, ctx->context_len,
...
CK_RV gm11tok_decrypt(STDLL_TokData_t *tokdata, SESSION *session,
    ...
    rc = dll_gm_Decrypt(ctx->context, ctx->context_len, input_data,
...
CK_RV gm11tok_decrypt_update(STDLL_TokData_t *tokdata, SESSION *session,
    ...
    rc = dll_gm_DecryptUpdate(ctx->context, ctx->context_len,
                              input_part, input_part_len, output_part,
                              p_output_part_len, (uint64_t)gm11_data->target_list);
...
CK_RV gm11tok_encrypt_final(STDLL_TokData_t *tokdata, SESSION *session,
    ...
    rc = dll_gm_EncryptFinal(ctx->context, ctx->context_len,
                             output_part, p_output_part_len,
                             (uint64_t)gm11_data->target_list);
...
CK_RV gm11tok_encrypt(STDLL_TokData_t *tokdata, SESSION *session,
    ...
    rc = dll_gm_Encrypt(ctx->context, ctx->context_len, input_data,
                        input_data_len, output_data, p_output_data_len,
                        (uint64_t)gm11_data->target_list);
...
CK_RV gm11tok_encrypt_update(STDLL_TokData_t *tokdata, SESSION *session,
    ...
    rc = dll_gm_EncryptUpdate(ctx->context, ctx->context_len,
                              input_part, input_part_len, output_part,
                              p_output_part_len,
                              (uint64_t)gm11_data->target_list);
...
static CK_RV gm11_ende_crypt_init(STDLL_TokData_t *tokdata, SESSION *session,
    ...
    size_t blob_len = 0;
    OBJECT *key_obj = NULL;
    size_t gm11_state_l = MAX_CRYPT_STATE_BYTES;
    CK_BYTE *gm11_state = malloc(gm11_state_l); /* freed by encr/decr_mgr.c */
    ...
    if (op == DECRYPT) {
        ENCR_DECR_CONTEXT *ctx = &session->decr_ctx;
        rc = dll_gm_DecryptInit(gm11_state, &gm11_state_l, mech, blob,
                                blob_len, (uint64_t)gm11_data->target_list);
        ctx->key = key;
        ctx->active = TRUE;
        ctx->context = gm11_state;
        ctx->context_len = gm11_state_l;
        if (rc != CKR_OK) {
            TRACE_ERROR("%s gm_DecryptInit rc=0x%lx blob_len=0x%zx "
                        "mech=0x%lx\n", __func__, rc, blob_len,
                        mech->mechanism);
        } else {
            TRACE_INFO("%s gm_DecryptInit rc=0x%lx blob_len=0x%zx "
                       "mech=0x%lx\n", __func__, rc, blob_len,
                       mech->mechanism);
        }
    } else {
        ENCR_DECR_CONTEXT *ctx = &session->encr_ctx;
        rc = dll_gm_EncryptInit (gm11_state, &gm11_state_l, mech, blob,
                                 blob_len, (uint64_t)gm11_data->target_list);
        ctx->key = key;
        ctx->active = TRUE;
        ctx->context = gm11_state;
        ctx->context_len = gm11_state_l;
        if (rc != CKR_OK) {
            TRACE_ERROR("%s gm_EncryptInit rc=0x%lx blob_len=0x%zx "
                        "mech=0x%lx\n", __func__, rc, blob_len,
                        mech->mechanism);
        } else {
            TRACE_INFO("%s gm_EncryptInit rc=0x%lx blob_len=0x%zx "
                       "mech=0x%lx\n", __func__, rc, blob_len,
                       mech->mechanism);
        }
    }
...
CK_RV gm11tok_encrypt_init(STDLL_TokData_t *tokdata, SESSION *session,
    ...
    rc = gm11_ende_crypt_init(tokdata, session, mech, key, ENCRYPT);
...
CK_RV gm11tok_decrypt_init(STDLL_TokData_t *tokdata, SESSION *session,
    ...
    rc = gm11_ende_crypt_init(tokdata, session, mech, key, DECRYPT);
...
CK_RV gm11tok_wrap_key(STDLL_TokData_t *tokdata, SESSION *session,
     ...
     */
    rc = dll_gm_WrapKey(wrap_target_blob, wrap_target_blob_len, wrapping_blob,
                        wrapping_blob_len, NULL, ~0, mech, wrapped_key,
                        p_wrapped_key_len, (uint64_t)gm11_data->target_list);

    if (rc != CKR_OK) {
        TRACE_ERROR("%s gm_WrapKey failed with rc=0x%lx\n", __func__, rc);
    } else {
...
CK_RV gm11tok_unwrap_key(STDLL_TokData_t *tokdata, SESSION *session, CK_MECHANISM_PTR mech,
                         CK_ATTRIBUTE_PTR attrs, CK_ULONG attrs_len,
     ...
     */
    rc = dll_gm_UnwrapKey(wrapped_key, wrapped_key_len, wrapping_blob,
                          wrapping_blob_len, NULL, ~0, gm11_data->gm11_pin_blob,
                          gm11_data->gm11_pin_blob_len, mech, new_attrs, new_attrs_len,
                          keyblob, &keyblobsize, csum, &cslen,
                          (uint64_t)gm11_data->target_list);

    if (rc != CKR_OK) {
        TRACE_ERROR("%s gm_UnwrapKey rc=0x%lx blobsize=0x%zx mech=0x%lx\n",
                    __func__, rc, keyblobsize, mech->mechanism);
        goto error;
    }
    TRACE_INFO("%s gm_UnwrapKey rc=0x%lx blobsize=0x%zx mech=0x%lx\n",
               __func__, rc, keyblobsize, mech->mechanism);
    ...
    TRACE_INFO("%s gm_UnwrapKey length 0x%hhx 0x%hhx 0x%hhx 0x%hhx 0x%lx\n",
               __func__, csum[3], csum[4], csum[5], csum[6], len);

    /* Get the keytype to use when creating the key object */
    rc = gm11_get_keytype(new_attrs, new_attrs_len, mech, &ktype, &class);
    if (rc != CKR_OK) {
        TRACE_ERROR("%s get_subclass failed with rc=0x%lx\n", __func__, rc);
        goto error;
    }
...
const CK_MECHANISM_TYPE gm11_banned_mech_list[] = {
...
const CK_ULONG banned_mech_list_len = (sizeof(gm11_banned_mech_list) / sizeof(CK_MECHANISM_TYPE));
...
CK_RV gm11tok_get_mechanism_list(STDLL_TokData_t *tokdata,
    ...
    if (pMechanismList == NULL) {
        rc = dll_gm_GetMechanismList(0, pMechanismList, pulCount,
                                     (uint64_t)gm11_data->target_list);
        if (rc != CKR_OK) {
            TRACE_ERROR("%s bad rc=0x%lx from gm_GetMechanismList() #1\n", __func__, rc);
            return rc;
        }
        ...
        rc = dll_gm_GetMechanismList(0, mlist, &counter, (uint64_t)gm11_data->target_list);
        if (rc != CKR_OK) {
            TRACE_ERROR("%s bad rc=0x%lx from gm_GetMechanismList() #2\n", __func__, rc);
            free(mlist);
            return rc;
        }
        ...
        for (i = 0; i < counter; i++) {
            ...
            for (j = 0; j < banned_mech_list_len; j++) {
                if (mlist[i] == gm11_banned_mech_list[j]) {
                    banned = 1;
                    TRACE_INFO("%s banned mech '%s'\n",
                               __func__, gm11_get_ckm(gm11_banned_mech_list[j]));
                }
            }
            if (banned == 1) {
                /* banned mech found,
        ...
        /* find out size gm11 will report, cannot use the size
         * that comes as parameter, this is a 'reduced size',
         * gm11 would complain about insufficient list size
         */
        rc = dll_gm_GetMechanismList(0, mlist, &counter,
                                     (uint64_t)gm11_data->target_list);
        if (rc != CKR_OK) {
            TRACE_ERROR("%s bad rc=0x%lx from gm_GetMechanismList() #3\n", __func__, rc);
            return rc;
        }
        ...
        rc = dll_gm_GetMechanismList(0, mlist, &counter, (uint64_t)gm11_data->target_list);
        if (rc != CKR_OK) {
            TRACE_ERROR("%s bad rc=0x%lx from gm_GetMechanismList() #4\n", __func__, rc);
            free(mlist);
            return rc;
        }
...
CK_RV gm11tok_get_mechanism_info(STDLL_TokData_t *tokdata,
    ...
    rc = dll_gm_GetMechanismInfo(0, type, pInfo, (uint64_t)gm11_data->target_list);
    if (rc != CKR_OK) {
        TRACE_ERROR("%s gm_GetMechanismInfo(0x%lx) failed with rc=0x%lx\n",
                    __func__, type, rc);
        return rc;
    }
    ...
    for (i = 0; i < banned_mech_list_len; i++) {
        if (type == gm11_banned_mech_list[i])
            return CKR_MECHANISM_INVALID;
    }
#ifdef DEFENSIVE_MECHLIST
    if (rc == CKR_OK) {
 ...
 */
static inline short check_n(gm11_target_t *target, char *nptr, int *apqn_i) {
    int num;
...
static int read_adapter_config_file(STDLL_TokData_t *tokdata, const char* conf_name) {
    ...
    char fname[PATH_MAX];
    gm11_target_t *gm11_targets = (gm11_target_t *)gm11_data->target_list;
    int rc = 0;
     ...
     * directory where the gm11 token config file is searched.
     */
    if (conf_dir) {
    ...
    /* now we should really have an open gm11 token config file */
    if (!ap_fp) {
        TRACE_ERROR("%s no valid EP 11 config file found\n", __func__);
        return APQN_FILE_INV_2;
    }

    TRACE_INFO("%s GM 11 token config file is '%s'\n", __func__, fname);
    ...
    for (i=0, str=filebuf; rc == 0; str=NULL) {
        ...
        if (i == 0) {
            ...
            if (strncmp(token, "END", 3) == 0)
                ...
                if (check_n(gm11_targets, token, &apqn_i) < 0) {
            ...
            gm11_targets->length++;
            if (gm11_targets->length > MAX_APQN) {
    ...
    if (rc == 0 && whitemode) {
        TRACE_INFO("%s whitelist with %d APQNs defined:\n",
                   __func__, gm11_targets->length);
        for (i=0; i < gm11_targets->length; i++) {
            TRACE_INFO(" APQN entry %d: adapter=%d domain=%d\n", i,
                       gm11_targets->apqns[2*i],
                       gm11_targets->apqns[2*i+1]);
        }
    }
