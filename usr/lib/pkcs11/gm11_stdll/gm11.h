...
CK_RV gm_GenerateRandom   (CK_BYTE_PTR rnd, CK_ULONG len,     uint64_t target) ;
/**/
/* note: external seeding not supported */
CK_RV gm_SeedRandom (CK_BYTE_PTR pSeed,   CK_ULONG ulSeedLen,
                       uint64_t target) ;
CK_RV gm_DigestInit     (unsigned char *state,     size_t *len,
               const CK_MECHANISM_PTR pmech,
                             uint64_t target) ;
/**/
CK_RV gm_Digest (const unsigned char *state,       size_t slen,
                        CK_BYTE_PTR data,       CK_ULONG len,
                        CK_BYTE_PTR digest, CK_ULONG_PTR dglen,
                           uint64_t target) ;
CK_RV gm_DigestUpdate (unsigned char *state,       size_t slen,
                        CK_BYTE_PTR data,       CK_ULONG dlen,
                           uint64_t target) ;
CK_RV gm_DigestKey    (unsigned char *state,       size_t slen,
                const unsigned char *key,         size_t klen,
                           uint64_t target) ;
CK_RV gm_DigestFinal  (const unsigned char *state,       size_t slen,
                              CK_BYTE_PTR digest, CK_ULONG_PTR dlen,
                                 uint64_t target) ;
CK_RV gm_DigestSingle (CK_MECHANISM_PTR pmech,
                           CK_BYTE_PTR data,       CK_ULONG len,
                           CK_BYTE_PTR digest, CK_ULONG_PTR dlen,
                              uint64_t target) ;

CK_RV gm_EncryptInit        (unsigned char *state, size_t *slen,
                         CK_MECHANISM_PTR pmech,
                      const unsigned char *key,   size_t klen,
                                 uint64_t target) ;
CK_RV gm_DecryptInit        (unsigned char *state, size_t *slen,
                         CK_MECHANISM_PTR pmech,
                      const unsigned char *key,   size_t klen,
                                 uint64_t target) ;
/**/
CK_RV gm_EncryptUpdate      (unsigned char *state,       size_t slen,
                              CK_BYTE_PTR plain,      CK_ULONG plen,
                              CK_BYTE_PTR cipher, CK_ULONG_PTR clen,
                                 uint64_t target) ;
CK_RV gm_DecryptUpdate      (unsigned char *state,       size_t slen,
                              CK_BYTE_PTR cipher,     CK_ULONG clen,
                              CK_BYTE_PTR plain,  CK_ULONG_PTR plen,
                                 uint64_t target) ;
/**/
/* one-pass en/decrypt with key blob */
CK_RV gm_Encrypt       (const unsigned char *state,       size_t slen,
                               CK_BYTE_PTR plain,      CK_ULONG plen,
                               CK_BYTE_PTR cipher, CK_ULONG_PTR clen,
                                  uint64_t target) ;
CK_RV gm_Decrypt       (const unsigned char *state,       size_t slen,
                               CK_BYTE_PTR cipher,     CK_ULONG clen,
                               CK_BYTE_PTR plain,  CK_ULONG_PTR plen,
                                  uint64_t target) ;
/**/
CK_RV gm_EncryptFinal  (const unsigned char *state,       size_t slen,
                               CK_BYTE_PTR output, CK_ULONG_PTR len,
                                  uint64_t target) ;
CK_RV gm_DecryptFinal  (const unsigned char *state,       size_t slen,
                               CK_BYTE_PTR output, CK_ULONG_PTR len,
                                  uint64_t target) ;
/**/
/* en/decrypt directly with key blob */
CK_RV gm_EncryptSingle (const unsigned char *key,         size_t klen,
                          CK_MECHANISM_PTR mech,
                               CK_BYTE_PTR plain,      CK_ULONG plen,
                               CK_BYTE_PTR cipher, CK_ULONG_PTR clen,
                                  uint64_t target) ;
CK_RV gm_DecryptSingle (const unsigned char *key,         size_t klen,
                          CK_MECHANISM_PTR mech,
                               CK_BYTE_PTR cipher,     CK_ULONG clen,
                               CK_BYTE_PTR plain,  CK_ULONG_PTR plen,
                                  uint64_t target) ;
...
CK_RV gm_ReencryptSingle (const unsigned char *dkey,     size_t dklen,
...
CK_RV gm_GenerateKey (CK_MECHANISM_PTR pmech,
                  ...
                  const unsigned char *pin,     size_t pinlen,
                        ...
                        unsigned char *csum,    size_t *clen,
                             uint64_t target) ;
/**/
CK_RV gm_GenerateKeyPair (CK_MECHANISM_PTR pmech,
...
CK_RV gm_SignInit     (unsigned char *state,     size_t *slen,
                   CK_MECHANISM_PTR alg,
                const unsigned char *key,       size_t klen,
                           uint64_t target) ;
CK_RV gm_VerifyInit   (unsigned char *state,     size_t *slen,
                   CK_MECHANISM_PTR alg,
                const unsigned char *key,       size_t klen,
                           uint64_t target) ;
/**/
CK_RV gm_SignUpdate   (unsigned char *state,     size_t slen,
                        CK_BYTE_PTR data,     CK_ULONG dlen,
                           uint64_t target) ;
CK_RV gm_VerifyUpdate (unsigned char *state,     size_t slen,
                        CK_BYTE_PTR data,     CK_ULONG dlen,
                           uint64_t target) ;
/**/
CK_RV gm_SignFinal    (const unsigned char *state,      size_t stlen,
                              CK_BYTE_PTR sig,   CK_ULONG_PTR siglen,
                                 uint64_t target) ;
CK_RV gm_VerifyFinal  (const unsigned char *state,      size_t stlen,
                              CK_BYTE_PTR sig,       CK_ULONG siglen,
                                 uint64_t target) ;
/**/
CK_RV gm_Sign   (const unsigned char *state,     size_t stlen,
                        CK_BYTE_PTR data,     CK_ULONG dlen,
                        CK_BYTE_PTR sig,  CK_ULONG_PTR siglen,
                           uint64_t target) ;
CK_RV gm_Verify (const unsigned char *state,     size_t stlen,
                        CK_BYTE_PTR data,     CK_ULONG dlen,
                        CK_BYTE_PTR sig,      CK_ULONG siglen,
                           uint64_t target) ;
/**/
CK_RV gm_SignSingle   (const unsigned char *key,      size_t klen,
                         CK_MECHANISM_PTR pmech,
                              CK_BYTE_PTR data,    CK_ULONG dlen,
                              CK_BYTE_PTR sig, CK_ULONG_PTR slen,
                                 uint64_t target) ;
CK_RV gm_VerifySingle (const unsigned char *key,      size_t klen,
                         CK_MECHANISM_PTR pmech,
                              CK_BYTE_PTR data,    CK_ULONG dlen,
...
CK_RV gm_WrapKey (const unsigned char *key,          size_t keylen,
...
/**/
/* mackey is NULL for PKCS#11 formats, not for authenticated ones */
CK_RV gm_UnwrapKey (const   CK_BYTE_PTR wrapped,  CK_ULONG wlen,
                   const unsigned char *kek,       size_t keklen,
                   const unsigned char *mackey,    size_t mklen,
                   const unsigned char *pin,       size_t pinlen,
...
CK_RV gm_DeriveKey ( CK_MECHANISM_PTR pderivemech,
                    CK_ATTRIBUTE_PTR ptempl, CK_ULONG templcount,
...
/**/
CK_RV gm_GetMechanismList (CK_SLOT_ID slot,
               CK_MECHANISM_TYPE_PTR mechs,
                        CK_ULONG_PTR count,
                            uint64_t target) ;
CK_RV gm_GetMechanismInfo (CK_SLOT_ID slot,
                   CK_MECHANISM_TYPE mech,
               CK_MECHANISM_INFO_PTR pmechinfo,
                            uint64_t target) ;

CK_RV gm_GetAttributeValue (const unsigned char *obj,        size_t olen,
                              CK_ATTRIBUTE_PTR pTemplate, CK_ULONG ulCount,
                                      uint64_t target) ;
CK_RV gm_SetAttributeValue       (unsigned char *obj,        size_t olen,
                              CK_ATTRIBUTE_PTR pTemplate, CK_ULONG ulCount,
                                      uint64_t target) ;


CK_RV gm_Login ( CK_UTF8CHAR_PTR pin,      CK_ULONG pinlen,
            const unsigned char *nonce,     size_t nlen,
                  unsigned char *pinblob,   size_t *pinbloblen,
                       uint64_t target) ;
CK_RV gm_Logout ( const unsigned char *pin, size_t len,     uint64_t target) ;

CK_RV gm_admin (unsigned char *response1, size_t *r1len,
 ...
 *  Module management.
 */
int gm_add_backend(const char *name, unsigned int port) ;
int gm_init     (void) ;
int gm_shutdown (void) ;
