...
CK_RV valid_mech(STDLL_TokData_t *tokdata, CK_MECHANISM_PTR m, CK_FLAGS f)
{
    ...
    if (m) {
        ...
        rc = gm11tok_get_mechanism_info(tokdata, m->mechanism, &info);
            ...
            SLOT_INFO *sinfp, struct trace_handle_t t)
{
    ...
    if (sltp->TokData->initialized == FALSE) {
        ...
        rc =  gm11tok_init(sltp->TokData, SlotNumber, sinfp->confname);
        if (rc != 0) {
            sltp->FcnList = NULL;
            if (sltp->TokData)
                free(sltp->TokData);
                          ...
                          CK_ULONG_PTR count)
{
    ...
    rc = gm11tok_get_mechanism_list(tokdata, pMechList, count);
    if (rc == CKR_OK) {
              ...
              CK_MECHANISM_TYPE type, CK_MECHANISM_INFO_PTR pInfo)
{
    ...
    rc = gm11tok_get_mechanism_info(tokdata, type, pInfo);
out:
             ...
             CK_MECHANISM_PTR pMechanism, CK_OBJECT_HANDLE hKey)
{
    ...
    rc = gm11tok_encrypt_init(tokdata, sess, pMechanism, hKey);
         ...
         CK_BYTE_PTR pEncryptedData, CK_ULONG_PTR pulEncryptedDataLen)
{
    ...
    rc = gm11tok_encrypt(tokdata, sess, pData, ulDataLen, pEncryptedData,
                 pulEncryptedDataLen);
    if (rc != CKR_OK)
        TRACE_DEVEL("gm11tok_encrypt() failed.\n");
               ...
               CK_ULONG_PTR pulEncryptedPartLen)
{
    ...
    rc = gm11tok_encrypt_update(tokdata, sess, pPart, ulPartLen, pEncryptedPart,
                    pulEncryptedPartLen);
    if (rc != CKR_OK)
        TRACE_DEVEL("gm11tok_encrypt_update() failed.\n");
              ...
              CK_ULONG_PTR pulLastEncryptedPartLen)
{
    ...
    rc = gm11tok_encrypt_final(tokdata, sess, pLastEncryptedPart,
                   pulLastEncryptedPartLen);
    if (rc != CKR_OK)
        TRACE_ERROR("gm11tok_encrypt_final() failed.\n");
             ...
             CK_MECHANISM_PTR pMechanism, CK_OBJECT_HANDLE hKey)
{
    ...
    rc = gm11tok_decrypt_init(tokdata, sess, pMechanism, hKey);
    if (rc != CKR_OK)
        TRACE_DEVEL("gm11tok_decrypt_init() failed.\n");
         ...
         CK_BYTE_PTR pData, CK_ULONG_PTR pulDataLen)
{
    ...
    rc = gm11tok_decrypt(tokdata, sess, pEncryptedData, ulEncryptedDataLen,
                 pData, pulDataLen);
    if (rc != CKR_OK)
        TRACE_DEVEL("gm11tok_decrypt() failed.\n");
               ...
               CK_BYTE_PTR pPart, CK_ULONG_PTR pulPartLen)
{
    ...
    rc = gm11tok_decrypt_update(tokdata, sess, pEncryptedPart,
                    ulEncryptedPartLen, pPart, pulPartLen);
    if (rc != CKR_OK)
        TRACE_DEVEL("gm11tok_decrypt_update() failed.\n");
              ...
              CK_BYTE_PTR pLastPart, CK_ULONG_PTR pulLastPartLen)
{
    ...
    rc = gm11tok_decrypt_final(tokdata, sess, pLastPart, pulLastPartLen);
    if (rc != CKR_OK)
        TRACE_DEVEL("gm11tok_decrypt_final() failed.\n");
done:
    if (rc != CKR_BUFFER_TOO_SMALL && (rc != CKR_OK || length_only != TRUE))
        decr_mgr_cleanup( &sess->decr_ctx );
          ...
          CK_MECHANISM_PTR pMechanism, CK_OBJECT_HANDLE hKey)
{
    ...
    rc = gm11tok_sign_init(tokdata, sess, pMechanism, FALSE, hKey);
    if (rc != CKR_OK)
          ...
          CK_ULONG_PTR pulSignatureLen)
{
    ...
    rc = gm11tok_sign(tokdata, sess, length_only, pData, ulDataLen, pSignature,
              pulSignatureLen);
    if (rc != CKR_OK)
        TRACE_DEVEL("gm11tok_sign() failed.\n");
            ...
            CK_BYTE_PTR pPart, CK_ULONG ulPartLen)
{
    ...
    rc = gm11tok_sign_update(tokdata, sess, pPart, ulPartLen);
    if (rc != CKR_OK)
        TRACE_DEVEL("gm11tok_sign_update() failed.\n");
           ...
           CK_BYTE_PTR pSignature, CK_ULONG_PTR pulSignatureLen)
{
    ...
    rc = gm11tok_sign_final(tokdata, sess, length_only, pSignature,
                pulSignatureLen);
    if (rc != CKR_OK)
        TRACE_ERROR("gm11tok_sign_final() failed.\n");
            ...
            CK_MECHANISM_PTR pMechanism, CK_OBJECT_HANDLE hKey)
{
    ...
    rc = gm11tok_verify_init(tokdata, sess, pMechanism, FALSE, hKey);
    if (rc != CKR_OK)
        TRACE_DEVEL("gm11tok_verify_init() failed.\n");
        ...
        CK_ULONG ulSignatureLen)
{
    ...
    rc = gm11tok_verify(tokdata, sess, pData, ulDataLen, pSignature,
                ulSignatureLen);
    if (rc != CKR_OK)
        TRACE_DEVEL("gm11tok_verify() failed.\n");
              ...
              CK_BYTE_PTR pPart, CK_ULONG ulPartLen)
{
    ...
    rc = gm11tok_verify_update(tokdata, sess, pPart, ulPartLen);
    if (rc != CKR_OK)
        TRACE_DEVEL("gm11tok_verify_update() failed.\n");
             ...
             CK_BYTE_PTR pSignature, CK_ULONG ulSignatureLen)
{
    ...
    rc = gm11tok_verify_final(tokdata, sess, pSignature, ulSignatureLen);
    if (rc != CKR_OK)
        TRACE_DEVEL("gm11tok_verify_final() failed.\n");
             ...
             CK_ULONG ulCount, CK_OBJECT_HANDLE_PTR phKey)
{
    ...
    rc = gm11tok_generate_key(tokdata, sess, pMechanism, pTemplate,
                  ulCount, phKey);
    if (rc != CKR_OK)
        TRACE_DEVEL("gm11tok_generate_key() failed.\n");
             ...
             CK_OBJECT_HANDLE_PTR phPrivateKey)
{
    ...
    rc = gm11tok_generate_key_pair(tokdata, sess, pMechanism,
                       pPublicKeyTemplate,
    ...
    if (rc != CKR_OK)
        TRACE_DEVEL("gm11tok_generate_key_pair() failed.\n");
         ...
         CK_BYTE_PTR pWrappedKey, CK_ULONG_PTR pulWrappedKeyLen)
{
    ...
    rc = gm11tok_wrap_key(tokdata, sess, pMechanism, hWrappingKey, hKey,
                  pWrappedKey, pulWrappedKeyLen);
    if (rc != CKR_OK)
        TRACE_DEVEL("gm11tok_wrap_key() failed.\n");
           ...
           CK_ULONG ulCount, CK_OBJECT_HANDLE_PTR phKey)
{
    ...
    rc = gm11tok_unwrap_key(tokdata, sess, pMechanism, pTemplate, ulCount,
                pWrappedKey, ulWrappedKeyLen, hUnwrappingKey,
                phKey);
    if (rc != CKR_OK)
        TRACE_DEVEL("gm11tok_unwrap_key() failed.\n");
           ...
           CK_ULONG ulCount, CK_OBJECT_HANDLE_PTR phKey)
{
    ...
    rc = gm11tok_derive_key(tokdata, sess, pMechanism, hBaseKey, phKey,
                pTemplate, ulCount);
    if (rc != CKR_OK)
