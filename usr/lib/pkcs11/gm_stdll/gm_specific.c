...
***************************************************************************
                          Change Log
                          ==========
       01/16/2018  Guomi SM2/SM3/SM4 support
...
CK_CHAR manuf[] = "Softway Corp.";
CK_CHAR model[] = "Softway GMTok ";
CK_CHAR descr[] = "Softway PKCS#11 GM token";
...
token_specific_init(STDLL_TokData_t *tokdata, CK_SLOT_ID SlotNumber,
		    char *conf_name)
{
	TRACE_INFO("guomi %s slot=%lu running\n", __func__, SlotNumber);
...
/*
    ...
    if (rc == FALSE){
        TRACE_ERROR("Could not find CKA_IBM_OPAQUE in the template.\n");
        return CKR_FUNCTION_FAILED;
    } else {
        privblob = attr->pValue;
        privblobsize = attr->ulValueLen;
        TRACE_INFO("Private key size: %d\n", privblobsize);
        TRACE_DEVEL_HEX("Private key value", privblob, privblobsize);
    }

    EVP_PKEY *pkey = d2i_PrivateKey(type, &ec_key, privblob, privblobsize);
    if (pkey = NULL) {
        TRACE_ERROR("Error: d2i_PrivateKey, code:%d \n", ERR_get_error());
        return CKR_FUNCTION_FAILED;
    }
...
CK_RV
token_specific_ec_generate_keypair( STDLL_TokData_t *tokdata,
                     TEMPLATE  * publ_tmpl,
                     TEMPLATE  * priv_tmpl )
{
    CK_RV                rc;

    rc = os_specific_ec_keygen(publ_tmpl, priv_tmpl);
    if (rc != CKR_OK)
        TRACE_DEVEL("os_specific_ec_keygen failed\n");
    return rc;
}
                 ...
                 OBJECT    * key_obj )
{
	...
	// Convert the local key representation to an RSA key representation
...
static CK_RV gmtok_hmac_init(STDLL_TokData_t *tokdata, SIGN_VERIFY_CONTEXT *ctx,
...
{
	return gmtok_hmac_init(tokdata, &sess->sign_ctx, mech, Hkey);
}

CK_RV token_specific_hmac_verify_init (STDLL_TokData_t *tokdata, SESSION *sess,
				       CK_MECHANISM *mech, CK_OBJECT_HANDLE Hkey)
{
	return gmtok_hmac_init(tokdata, &sess->verify_ctx, mech, Hkey);
}

static CK_RV gmtok_hmac(SIGN_VERIFY_CONTEXT *ctx, CK_BYTE *in_data,
                          CK_ULONG in_data_len, CK_BYTE *signature,
...
CK_RV token_specific_hmac_sign(STDLL_TokData_t *tokdata, SESSION *sess,
			       ...
			       CK_BYTE *signature, CK_ULONG *sig_len)
{
	return gmtok_hmac(&sess->sign_ctx, in_data, in_data_len, signature,
			    sig_len, TRUE);
}
...
CK_RV token_specific_hmac_verify(STDLL_TokData_t *tokdata, SESSION *sess,
				 ...
				 CK_BYTE *signature, CK_ULONG sig_len)
{
	return gmtok_hmac(&sess->verify_ctx, in_data, in_data_len, signature,
                            &sig_len, FALSE);
}

static CK_RV gmtok_hmac_update(SIGN_VERIFY_CONTEXT *ctx, CK_BYTE *in_data,
...
CK_RV token_specific_hmac_sign_update(STDLL_TokData_t *tokdata, SESSION *sess,
				      CK_BYTE *in_data, CK_ULONG in_data_len)
{
	return gmtok_hmac_update(&sess->sign_ctx, in_data, in_data_len, TRUE);
}

CK_RV token_specific_hmac_verify_update(STDLL_TokData_t *tokdata, SESSION *sess,
					CK_BYTE *in_data, CK_ULONG in_data_len)
{
	return gmtok_hmac_update(&sess->verify_ctx, in_data, in_data_len,
				   FALSE);
}

static CK_RV gmtok_hmac_final(SIGN_VERIFY_CONTEXT *ctx, CK_BYTE *signature,
				CK_ULONG *sig_len, CK_BBOOL sign)
{
	int rc;
	size_t mac_len, len;
...
CK_RV token_specific_hmac_sign_final(STDLL_TokData_t *tokdata, SESSION *sess,
				     CK_BYTE *signature, CK_ULONG *sig_len)
{
	return gmtok_hmac_final(&sess->sign_ctx, signature, sig_len, TRUE);
}

CK_RV token_specific_hmac_verify_final(STDLL_TokData_t *tokdata, SESSION *sess,
				       CK_BYTE *signature, CK_ULONG sig_len)
{
	return gmtok_hmac_final(&sess->verify_ctx, signature, &sig_len, FALSE);
}
