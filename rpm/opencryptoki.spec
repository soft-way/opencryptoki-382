...
%package gmtok
Group:			System Environment/Libraries
Summary:		The guomi token implementation for opencryptoki
Requires(pre):		%{name}-libs%{?_isa} = %{version}-%{release}
Requires:		%{name}-libs%{?_isa} = %{version}-%{release}
Requires:		gmssl-devel
Provides:		%{name}(token)

%description gmtok
Opencryptoki implements the PKCS#11 specification v2.20 for a set of
cryptographic hardware, such as IBM 4764 and 4765 crypto cards, and the
Trusted Platform Module (TPM) chip. This is part of Guomi software
token implementation that can be used without any cryptographic
hardware.
This package brings the guomi software token implementation to use opencryptoki
without any specific cryptographic hardware.

%package gm11tok
Group:          System Environment/Libraries
Summary:        GM11 support for opencryptoki
Requires(pre):      %{name}-libs%{?_isa} = %{version}-%{release}
Requires:       %{name}-libs%{?_isa} = %{version}-%{release}
Provides:       %{name}(token)
...
%description gm11tok
Opencryptoki implements the PKCS#11 specification v2.20 for a set of
cryptographic hardware, this is software guomi demo.
...
%configure --with-systemd=%{_unitdir} --with-openssl=/opt/gmssl \
%ifarch s390 s390x
...
%post gmtok -p /sbin/ldconfig
%post gm11tok -p /sbin/ldconfig
...
%postun gmtok -p /sbin/ldconfig
%postun gm11tok -p /sbin/ldconfig
...
%files gmtok
%{_libdir}/opencryptoki/stdll/libpkcs11_gm.*
%{_libdir}/opencryptoki/stdll/PKCS11_GM.so
%dir %attr(770,root,pkcs11) %{_sharedstatedir}/%{name}/gmtok/
%dir %attr(770,root,pkcs11) %{_sharedstatedir}/%{name}/gmtok/TOK_OBJ/

%files gm11tok
%config(noreplace) %{_sysconfdir}/%{name}/gm11tok.conf
%{_libdir}/opencryptoki/stdll/libpkcs11_gm11.*
%{_libdir}/opencryptoki/stdll/PKCS11_GM11.so
%dir %attr(770,root,pkcs11) %{_sharedstatedir}/%{name}/gm11tok/
%dir %attr(770,root,pkcs11) %{_sharedstatedir}/%{name}/gm11tok/TOK_OBJ/
...
%changelog
* Thu Jan 16 2018 Samuel Chen
- Add Guomi/Guomi11 token
